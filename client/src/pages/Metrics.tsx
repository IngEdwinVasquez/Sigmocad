...
export function Metrics() {
  ...
  const fetchAllData = async () => {
    ...
    try {
      const [metricsRes, creativesRes, slotsRes, mediaRes] = await Promise.all([
        api.get<Metric[]>('/api/metrics', { start: startDateTime, end: endDateTime, media_id: selectedMediaId }),
      ...
      processMetrics(metricsRes);
      setCreatives(creativesRes);
      setSlots(slotsRes);
      setMedia(mediaRes);

      processPerformanceData(metricsRes, creativesRes, slotsRes, mediaRes);
      processHourlyData(metricsRes);
      processDeviceData(metricsRes);
      processLocationData(metricsRes);
      processCityData(metricsRes);
  ...
  const processComparison = async (currentStart: string, currentEnd: string) => {
    ...
    const [current, previous] = await Promise.all([
      api.get<{ impressions: number; clicks: number }>('/api/metrics/counts', { start: currentStart, end: currentEnd, media_id: selectedMediaId }),
      api.get<{ impressions: number; clicks: number }>('/api/metrics/counts', { start: previousStart.toISOString(), end: previousEnd.toISOString(), media_id: selectedMediaId }),
    ]);

    setComparison({
      current: { impressions: current.impressions, clics: current.clicks },
      previous: { impressions: previous.impressions, clics: previous.clicks },
    });
  };
