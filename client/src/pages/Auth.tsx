...
export function Auth() {
  ...
  useEffect(() => {
    const loadCompanyFromURL = async () => {
      ...
      if (companySlug) {
        try {
          const data = await api.get<Company>(`/api/public/companies/by-slug/${encodeURIComponent(companySlug)}`);
          setCompany(data);
        } catch {
          setCompany(null);
        }
      ...
      <div className="max-w-md w-full relative">
        ...
        <div className="mt-6 text-center">
          <p className="text-slate-600 text-xs">
            © {new Date().getFullYear()} {companyName || 'SIGMOCAD'}. Todos los derechos reservados.
          </p>
        </div>
      </div>
    </div>
