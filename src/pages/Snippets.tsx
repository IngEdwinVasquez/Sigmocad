...
export function Snippets() {
  ...
  const supabaseUrl = normalizeUrl(supabaseUrlRaw);
  ...
  const fetchSlots = async (mediaId: string) => {
    const { data, error } = await supabase
      .from('slots')
      .select('id, media_id, slug, width, height')
      .eq('media_id', mediaId)
      .eq('status', 'ACTIVE')
      .order('slug');
  ...
  const snippet = useMemo(() => {
    if (!currentMedia || !currentSlot || !supabaseUrl) return '';
...
(function(){
  ...
  }, [currentMedia, currentSlot, supabaseUrl]);
  ...
  useEffect(() => {
    ...
    fetch(url, { cache: 'no-store', signal: ac.signal })
      .then(async (r) => {
        ...
        console.log('Embed response:', { status: r.status, hasContent: !!text.trim(), contentLength: text.length, text });
        if (!r.ok) {
