...
export function Slots() {
  ...
  const fetchData = async () => {
    ...
    const [slotsRes, mediaRes] = await Promise.all([
      supabase.from('slots').select('*, media(id, name)').order('created_at', { ascending: false }),
      supabase.from('media').select('id, name').eq('status', 'ACTIVE'),
    ]);
  ...
  const handleExcelUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!error) {
        await fetchData();
        alert(`Se importaron exitosamente ${slotsToInsert.length} espacios`);
      } else {
        alert('Error al importar espacios: ' + error.message);
      }
    } catch (error) {
